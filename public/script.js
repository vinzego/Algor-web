document.addEventListener('DOMContentLoaded', () => {
  ...
  // 15. Real-Time Upward Growing Apple Chat Loop (Automatske rezervacije)
  const appleChatBox = document.querySelector('.chat-apple-style-box');
  if (appleChatBox) {
    const chatMessages = [
      {
        type: 'left',
        text: 'Hej! Imate li slobodan stol za 4 osobe večeras u 20:00h?'
      },
      {
        type: 'right',
        text: 'Pozdrav! Imamo slobodan stol na terasi u 20:00h. Želite li da vam rezerviram?'
      },
      {
        type: 'left',
        text: 'Može, hvala! Na ime Marko.'
      },
      {
        type: 'right',
        confirm: true,
        text: 'Stol za 4 osobe u 20:00h je uspješno rezerviran! 🍷'
      }
    ];

    const runChatCycle = () => {
      appleChatBox.innerHTML = '';
      appleChatBox.style.transition = 'none';
      appleChatBox.style.transform = 'translateY(0)';
      appleChatBox.style.opacity = '1';
      ...
      chatMessages.forEach((msg) => {
        setTimeout(() => {
          ...
          row.className = `apple-msg-row msg-${msg.type} apple-pop-in`;

          if (msg.type === 'left') {
            row.innerHTML = `
              <div class="apple-avatar avatar-user">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              </div>
              <div class="apple-bubble bubble-left">
                <span>${msg.text}</span>
              </div>
            `;
          } else {
            row.innerHTML = `
              <div class="apple-bubble bubble-right ${msg.confirm ? 'bubble-confirm' : ''}">
                <span>${msg.text}</span>
              </div>
              <div class="apple-avatar avatar-bot">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a2 2 0 0 1 2 2v2h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h2V4a2 2 0 0 1 2-2z"/><circle cx="9" cy="12" r="1.5"/><circle cx="15" cy="12" r="1.5"/><path d="M9 16s1.5 1.5 3 1.5 3-1.5 3-1.5"/></svg>
              </div>
            `;
          }

          appleChatBox.appendChild(row);
        }, stepDelay);

        stepDelay += 1300;
      });

      // When confirmation message arrives (stepDelay ~5500ms), wait 2.5s and then slide ALL messages DOWN together
      setTimeout(() => {
        appleChatBox.style.transition = 'transform 0.75s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.75s ease';
        appleChatBox.style.transform = 'translateY(60px)';
        appleChatBox.style.opacity = '0';
